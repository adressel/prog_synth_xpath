...
object ian_test extends App {
  NodeInfo.populate
  println("NodeInfo preparation done")
//  NodeInfo.all.map(s => println(s"${s.id} + ${s.label}"))
  var good_ids = Vector(27)
  ...
  excutor.remaining_bad_ids = bad_ids.toList
  val cvs : ArrayBuffer[String] = ArrayBuffer()
  var num_parents = -1
  while (!excutor.get_isDone){
//	  println(excutor.remaining_bad_ids)
	  val temp =  excutor.children_excute(label_name, good_ids, bad_ids.toVector).distinct
	  excutor.forbidden_labels += label_name
	  ...
	  bad_ids = excutor.remaining_bad_ids.map(s => NodeInfo.get_map(s).parent_id).toSet
	  excutor.remaining_bad_ids = bad_ids.toList
	  num_parents += 1
  }
